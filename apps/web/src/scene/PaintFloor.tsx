...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Pale terrazzo: speckled off-white with a faint lane tint either side of centre.
    vec3 base = uCourt;
    float speck = step(0.86, hash(floor(vWorld.xz * 7.0)));
    base = mix(base, base * 0.86, speck * 0.5);
    base = mix(base, base * 1.05, noise(vWorld.xz * 0.35));
    ...
    // Glossy plastic sheen so wet paint reads as wet.
    float sheen = 0.06 * noise(vWorld.xz * 1.6 + uTime * 0.05);
...
export function PaintFloor({ world, highContrast }: Props) {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uCourt: { value: new THREE.Color('#e9e4d8') },
